    ...
    function apply(ctx) {
      ...
      styleEl.textContent = `
        html::before, html::after {
          ...
          pointer-events: none;
          z-index: -1;
          filter: blur(var(--dswpl-blur, 0px));
        }
        html::before {
          background-image:
      ...
      quickBtn.setAttribute("aria-label", "快捷开关壁纸");
      document.body.appendChild(quickBtn);

      function updateQuickBtn() {
        if (store.get().enabled) {
          quickBtn.textContent = "⏸ 关闭壁纸";
          quickBtn.classList.remove("dswpl-off");
          quickBtn.title = "立即关闭壁纸背景";
        } else {
          quickBtn.textContent = "▶ 开启壁纸";
          quickBtn.classList.add("dswpl-off");
          quickBtn.title = "壁纸已关闭，点击重新开启";
        }
      }
      updateQuickBtn();
      store.subscribe(updateQuickBtn);
      ...
      ctx.effect(() => () => {
        ...
        videoLayer.remove();
        quickBtn.remove();
        for (const v of ["--wp-cur", "--wp-cur-op", "--wp-next", "--wp-next-op", "--wp-dur", "--wp-scrim", "--dswpl-ts", "--dswpl-blur"]) {
